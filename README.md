...
1. Тапнуть правой кнопкой на [environment_recovery.desktop]() 
2. Сохранить через `Save as` (`Сохранить ссылку как`) на рабочем столе
3. Тапнуть два раза на файл `environment_recovery.desktop`
4. Ввести `sudo` пароль во всплывшем окне
5. Дождаться перезагрузки Steam Deck
